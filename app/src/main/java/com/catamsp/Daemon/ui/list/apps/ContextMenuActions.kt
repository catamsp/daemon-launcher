...
fun AbstractAppInfo.toggleDistracting() {
    val distracting: MutableSet<AbstractAppInfo> =
        LauncherPreferences.apps().distractingApps() ?: mutableSetOf()

    if (distracting.contains(this)) {
        distracting.remove(this)
    } else {
        distracting.add(this)
    }

    LauncherPreferences.apps().distractingApps(distracting)
}
